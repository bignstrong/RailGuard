...
- Владелец: логин `ADMIN_LOGIN` (по умолчанию `owner`), scrypt-хеш пароля в `ADMIN_PASSWORD_HASH` используется при первом входе, дальше запись живёт в таблице `AdminUser`. Остальные пользователи создаются в разделе «Пользователи» (роли: администратор, менеджер).
- Двухфакторная защита (TOTP) выключена по умолчанию; каждый пользователь включает её в «Настройках», администратор может выключить её пользователю, потерявшему телефон. Сессия 8 часов в cookie `__Host-admin_session` (HttpOnly, Secure, SameSite=Strict). 5 попыток входа за 15 минут с IP. Опционально `ADMIN_ALLOWED_IPS`.
